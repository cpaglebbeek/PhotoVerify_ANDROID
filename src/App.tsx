...
function App() {
  ...
  const runOneClickShield = async () => {
    ...
    if (useStamp && finalCode.length !== 6) { alert("Error: Code must be exactly 6 characters."); return; }
    ...
    if (targetWidth > MAX_DIM || targetHeight > MAX_DIM) {
      ...
      targetWidth = Math.floor(targetWidth * ratio); targetHeight = Math.floor(targetHeight * ratio);
    ...
    if (useBorder) {
      ctx.strokeStyle = 'rgba(96, 165, 250, 0.5)'; ctx.lineWidth = 1; ctx.strokeRect(0.5, 0.5, targetWidth - 1, targetHeight - 1);
      borderCanvas = document.createElement('canvas'); borderCanvas.width = targetWidth; borderCanvas.height = targetHeight;
      ...
      interiorCanvas = document.createElement('canvas'); interiorCanvas.width = targetWidth - 2; interiorCanvas.height = targetHeight - 2;
      interiorCanvas.getContext('2d')!.drawImage(canvas, 1, 1, targetWidth - 2, targetHeight - 2, 0, 0, targetWidth - 2, targetHeight - 2);
    } else {
      interiorCanvas = document.createElement('canvas'); interiorCanvas.width = targetWidth; interiorCanvas.height = targetHeight;
    ...
    if (useStamp) {
      ...
      if (useBorder) { ctx.drawImage(interiorCanvas, 1, 1); } else { ctx.drawImage(interiorCanvas, 0, 0); }
    }

    const dna = generatePerceptualHashDetailed(currentInteriorData);
    const hash = await sha256(currentInteriorData.data);
    const now = Date.now();
    const deed = { imageHash: hash, perceptualHash: dna.hash, anchorHash: "AUTO", anchorSource: "AUTO-Generated", timestamp: now, features: { border: useBorder, stamp: useStamp }, combinedProof: await generateCombinedProof(hash, "AUTO") };
    
    await bundleEvidence(canvas.toDataURL('image/png'), borderCanvas ? borderCanvas.toDataURL('image/png') : null, interiorCanvas.toDataURL('image/png'), deed, `${useStamp ? finalCode : 'NOSTAMP'}_${sharedFilename}`);
    
    endProc(); alert("ZIP Bundle Saved!"); setMode('START');
  };
        ...
        <div className="card-glass text-center" style={{ maxWidth: '450px', width: '100%' }}>
          ...
          <div style={{ background: '#020617', padding: '10px', borderRadius: '8px', marginBottom: '20px', textAlign: 'left', border: '1px solid #1e293b' }}>
            ...
            <div style={{ maxHeight: '150px', overflowY: 'auto', fontFamily: 'monospace', fontSize: '0.75rem', color: '#38bdf8' }}>
              {debugLogs.length === 0 ? <span style={{ color: '#475569' }}>Waiting for sync...</span> : debugLogs.map((log, i) => (<div key={i} style={{ marginBottom: '2px', borderLeft: '2px solid #0ea5e9', paddingLeft: '5px' }}>{log}</div>))}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '10px', flexDirection: 'column' }}>
            ...
            <button className="btn btn-nav btn-success" onClick={manualSync} disabled={isSyncing} style={{ width: '100%', padding: '12px' }}>{isSyncing ? '⌛ Syncing...' : '🔄 Sync with Server'}</button>
          </div>
        ...
        <div className="header-top">
          <div className="app-branding" onClick={() => setMode('START')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            <div style={{ textAlign: 'left' }}>
              <h1 style={{ fontSize: '1.8rem', lineHeight: '1' }}>{content.ui.title} <span style={{ color: '#10b981', fontSize: '0.8rem' }}>[STABLE_V1.2.8]</span></h1>
      ...
      <main className="wizard-container">
          ...
          <div className="card-glass text-left">
            ...
            <div style={{ background: 'rgba(0,0,0,0.4)', padding: '15px', borderRadius: '12px', marginBottom: '15px', border: '1px solid #334155' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '0.85rem' }}>
                ...
                <div><label style={{ color: '#94a3b8', display: 'block', fontSize: '0.7rem' }}>CODENAME</label><strong style={{ color: '#fbbf24' }}>Cleveland</strong></div>
              </div>
              <hr style={{ border: '0', borderTop: '1px solid #1e293b', margin: '10px 0' }} />
              <label style={{ color: '#94a3b8', display: 'block', fontSize: '0.7rem' }}>DEVICE OWNER / NAME</label><strong style={{ color: '#fff', fontSize: '0.9rem' }}>{deviceInfo.name || 'Unknown Device'}</strong><small style={{ display: 'block', color: '#94a3b8', fontSize: '0.7rem' }}>Model: {deviceInfo.model}</small>
              <label style={{ color: '#94a3b8', display: 'block', fontSize: '0.7rem', marginTop: '10px' }}>FORENSIC DEVICE ID</label><code style={{ fontSize: '0.85rem', color: '#60a5fa' }}>{license?.deviceHash || 'Detecting...'}</code>
        ...
        {mode === 'INFO' && (
          <div className="card-glass text-left">
            ...
            <div style={{ marginTop: '15px', background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '8px' }}><h4 style={{ color: '#10b981', margin: '0' }}>📜 Layer 4: Time-Anchor</h4><p style={{ fontSize: '0.85rem', marginTop: '5px' }}>Links your image hash to a public "Anchor" hash from today.</p></div>
        ...
        {mode === 'SETTINGS' && (
          <div className="card-glass">
            ...
            <div className="info-box mb-1" style={{ fontSize: '0.8rem', background: 'rgba(0,0,0,0.3)', padding: '10px', borderRadius: '8px' }}><p style={{ margin: 0 }}>Current Active Folder: <code style={{ color: '#60a5fa' }}>{safFolderUri || 'Internal Documents (Default)'}</code></p></div>
            ...
            <div style={{ marginTop: '20px', borderTop: '1px solid #334155', paddingTop: '15px' }}>
              ...
              <button className="btn btn-secondary" onClick={() => { localStorage.removeItem('saf_folder_uri'); alert("Storage folder reset to Internal Documents."); window.location.reload(); }} style={{ width: '100%', fontSize: '0.8rem' }}>Reset to Default</button>
            </div>
          </div>
        )}
        ...
        {mode === 'SHIELD_AUTO' && (
          <div className="card-glass text-center">
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', margin: '15px 0', background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '8px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><input type="checkbox" checked={useBorder} onChange={e => setUseBorder(e.target.checked)} /><span style={{ fontSize: '0.9rem' }}>Physical Border</span></label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><input type="checkbox" checked={useStamp} onChange={e => setUseStamp(e.target.checked)} /><span style={{ fontSize: '0.9rem' }}>Invisible Stamp</span></label>
            </div>
            {useStamp && (
              <div style={{ margin: '15px 0', background: 'rgba(96, 165, 250, 0.1)', padding: '15px', borderRadius: '8px', border: '1px solid #60a5fa' }}>
                ...
                <input type="text" value={sharedUid} onChange={e => { const val = e.target.value.toUpperCase().replace(/[^0-9A-F]/g, ''); if (val.length <= 6) setSharedUid(val); }} maxLength={6} style={{ width: '100%', background: '#000', color: '#fff', textAlign: 'center', fontSize: '1.5rem', letterSpacing: '5px', fontFamily: 'monospace', border: '1px solid #334155' }} />
              </div>
            )}
            <label className="file-dropzone mt-1"><input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) { setSharedFilename(file.name); const img = new Image(); img.onload = () => setSharedImage(img); img.src = URL.createObjectURL(file); } }} />{sharedImage ? <img src={sharedImage.src} style={{ maxWidth: '100%', maxHeight: '200px' }} /> : <span>Click to load photo</span>}</label>
        ...
        {mode === 'VERIFY' && (
          <div className="wizard-flow">
            ...
            <div className="card-glass" style={{ border: '2px solid #60a5fa' }}>
              <ZipVerifier initialFile={sharedZipBlob} onNativePick={openNativeFilePicker} onStart={startProc} onProgress={setProgress} onEnd={endProc} />
            </div>
